#!/bin/bash
# bags-claim-v1.sh - Claim fees from Custom Fee Vault V1 positions
#
# Usage: ./bags-claim-v1.sh [--dry-run]
#
# Options:
...
#
# V1 positions have a two-party fee split (Claimer A / Claimer B).
# Each party claims their share separately based on their side.
...
# Rent exempt amount for vault PDA
VAULT_RENT_EXEMPT=1398960
...
echo "Bags Custom Fee Vault V1 Claimer"
echo "================================="
...
# Filter for V1 positions only
V1_POSITIONS=$(echo "$BAGS_POSITIONS" | jq --arg pid "$FEE_SHARE_V1_PROGRAM" \
  '[.response[] | select(.isCustomFeeVault == true and .programId == $pid)]')
POSITION_COUNT=$(echo "$V1_POSITIONS" | jq 'length')

if [ "$POSITION_COUNT" = "0" ]; then
  echo "No V1 positions to claim."
  exit 0
fi

echo "Found $POSITION_COUNT V1 position(s)"
echo ""
...
echo "$V1_POSITIONS" | jq -c '.[]' | while read -r pos; do
  ...
  # V1 specific fields
  CLAIMER_A=$(echo "$pos" | jq -r '.customFeeVaultClaimerA')
  CLAIMER_B=$(echo "$pos" | jq -r '.customFeeVaultClaimerB')
  ...
  CLAIM_REQUEST=$(jq -n \
    ...
    --arg feeShareProgramId "$FEE_SHARE_V1_PROGRAM" \
    --arg virtualPoolAddress "$VIRTUAL_POOL" \
    --arg claimerA "$CLAIMER_A" \
    --arg claimerB "$CLAIMER_B" \
    --arg claimerSide "$CLAIMER_SIDE" \
    '{
      feeClaimer: $feeClaimer,
      tokenMint: $tokenMint,
      isCustomFeeVault: true,
      feeShareProgramId: $feeShareProgramId,
      customFeeVaultClaimerA: $claimerA,
      customFeeVaultClaimerB: $claimerB,
      customFeeVaultClaimerSide: $claimerSide
    }')
